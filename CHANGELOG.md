...
## 2026-04-21
### 修复场景练习页返回后的题集同步
- 从场景练习题页返回主场景页时，现在会继续后台请求服务端 latest 练习题集；本地缓存仍用于秒开，但不会阻止接口同步。
...
- 手动重新生成会创建新的题集，旧 run / attempt 保持原题集追溯；练习运行态写入前会校验题集属于当前用户和当前场景。

## 2026-04-20
### 场景生成新增“按句子生成”
- scenes 页的生成入口新增“按句子生成”模式，并调整为默认模式；现在可以直接输入一句印象深刻的英文句子，围绕这句话生成一个短场景。
- 场景生成链路新增模式区分：系统会分别处理“按句子生成”和“按情境生成”，其中按句子生成会把输入句子作为锚点保留在结果里。
- 生成面板同步做了小幅体验优化，包括模式切换顺序、默认选中项，以及输入框内边距和边框样式调整。
