...
更准确地说，项目北极星不是某个页面或链路名称，而是用户结果：

> 让每一次场景学习，都沉淀为用户在未来真实场景中能回忆、能使用、能迁移的表达资产。

`today -> scene -> chunks -> review` 是实现这个北极星的机制：`today` 负责把用户带回学习现场，`scene` 负责提供真实语境，`chunks` 负责把表达资产化，`review` 负责把资产拉回主动回忆和输出训练。讲项目时，可以先讲这句用户结果，再讲这些页面如何分工。

它的判断维度不是“学习体验是否更丰富”，而是表达被用户在未来再次使用的概率是否更高：
...
如果从北极星视角看，这条链路的判断标准不是“有没有更多功能入口”，而是每一步是否在提高表达被未来再次使用的概率，并继续推进到回忆、使用和迁移。
