...
- 匿名访客安全边界:`/share/scene/[slug]` 灰度入口走"localStorage UUID + X-Anonymous-Id 头"识别 + "每日轮换 salt 的 IP hash"防绕过 + 同 IP 当日 session 上限 + 搜索引擎爬虫识别四道防线;Redis `anon:quota:*` 命名空间与已登录用户 `quota:{userId}:*` 完全隔离;Supabase RLS 公开内容表显式 anon SELECT、用户态表 deny-by-default(SQL parser audit test 守护);`ALLOW_ANONYMOUS_TRIAL=false` 一键回退所有匿名分支到 `/login`,主链路零影响
...
- `匿名试用 + 灰度入口`:`/share/scene/[slug]` 受 `ALLOW_ANONYMOUS_TRIAL` env 总开关控制,身份/配额/RLS/漏斗/紧急关闭五层与已登录用户严格隔离;`tts_play` 走独立 `tts-playback-quota` 模块不进 `HighCostCapability`(零边际成本 capability 不污染高成本统计);8 个漏斗事件 server / client 双路上报,daily cron 聚合到 `daily_anon_cost_report`(转化率 + 单转化成本)
