...
- `练习` / `变体` 是次级阶段入口：只有用户已经到达对应阶段且已有 generated/completed practice set 或 variant set 时才展示。
- 次级 `练习` 入口只负责进入已有 practice view，或在 latest practice set 已 completed 时开启再练一轮；首次生成题目仍只能由当前下一步主 CTA 或练习页内部明确重新生成动作触发。
- 次级 `变体` 入口只负责进入已有 variants view / active variant，或在 latest variant set 已 completed 时开启再练一轮；首次生成变体仍只能由当前下一步主 CTA 触发。
