...
`scenes` 列表页顶部提供随机复习播放入口，用于把已学到一定程度的场景串成被动听力复习队列。

规则：

- 只有 `progressPercent >= 60` 的场景可以进入随机复习播放队列。
- 点击入口时只随机选择一个起点；后续播放按当前列表中的合格场景顺序推进。
- 播放到最后一个合格场景后，从第一个合格场景继续循环。
- 列表页不直接保存完整 scene full segments；播放前按需通过 `slug` 获取场景详情，再组装完整场景音频 payload。
- 若没有合格场景，入口不可启动，并提示完成 60% 以上场景后可播放。

维护约束：

- 不要把随机复习播放理解成“随机拆句串播”；本入口播放的是每个场景的完整 scene full 音频。
