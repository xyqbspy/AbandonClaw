...
- 注册页邮箱验证主体验是“发送验证码 -> 输入验证码 -> 提交注册”。
- 项目 6 位邮箱验证码是新注册主链路的邮箱验证依据；验证码校验通过且账号创建成功后，该 Supabase Auth 用户必须满足邮箱已验证判定。
- 新注册用户不得因为没有额外点击 Supabase Confirm email 而被重复拦到 `/verify-email`。
- `/auth/callback` 只作为旧账号、手工补救或未来 Supabase 邮件能力的兼容入口；成功或失败后都只能跳转到安全站内目标。
- 邮箱未验证的旧账号或异常账号只能停留在认证相关页面或 `/verify-email`。
- `/verify-email` 可以保留最小重发验证邮件入口作为兼容补救路径，但不再是新注册主体验的必需步骤。
...
### 3.8 匿名访问只走灰度路径，且必须独立四道防线

匿名访问（未持有 Supabase Auth session 的访客）默认仍被拒绝；只有当 `ALLOW_ANONYMOUS_TRIAL=true` 且访问路径在显式灰度白名单内时才放行。规则边界：

- **路径白名单**：当前仅 `/trial`、`/trial/scene/[slug]`、`/share/scene/[slug]` 与 `/api/anonymous/*` 走匿名分支；首页“试用”入口链接到 `/trial`，分享链接继续使用 `/share/scene/[slug]`。不得开放 `/today`、`/scenes`、`/scene` 等主入口匿名访问。主入口前缀（today / scenes / scene / review / chunks / progress / settings / lesson / admin）由 middleware `PROTECTED_PAGE_PREFIXES` 显式守护，匿名访问被强制重定向到 `/login`。该白名单不得在未补充身份/配额/RLS/漏斗四件套的情况下扩展。
...
- **试用练习只读**：`/trial/scene/[slug]` 可展示基于公开场景的预生成练习体验，允许前端本地作答和反馈，但不得调用练习生成、提交、practice run、progress 或加入复习写入接口。
...
- **匿名漏斗事件强校验**：8 个事件名硬编码 union type，未知名直接抛错防漏斗失真；事件经 `recordAnonymousFunnelEventSafe` fire-and-forget 写入 `anonymous_funnel_events`，失败只 warn 不影响业务。
- **Cache-Control 强制 private no-store**：所有 `/share/*` 与 `/trial/*` 路径由 middleware 统一注入 `Cache-Control: private, no-store`,避免阻断弹窗状态或 signed URL 被 CDN 缓存命中。
- **一键止血**：`ALLOW_ANONYMOUS_TRIAL=false` 即所有匿名分支跳过，行为回归"必须登录"；既有登录用户主链路零影响。
