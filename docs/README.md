...
- Fast Track 小改动只需要读入口规则、相关文件、必要索引和最小测试上下文
- Spec-Driven 变更按“索引定位 -> 相关链路 -> 领域规则 -> stable spec -> system-design -> 代码”的顺序递进阅读
- 不要批量通读整个目录来替代定位；历史 archive、旧 proposal 和 dev-log 只能作为历史参考，不能覆盖当前 stable spec
- 如果处理中发现主链路、状态流、数据流、缓存、权限或稳定规则风险，再补读对应上下文并重新判断任务类型
