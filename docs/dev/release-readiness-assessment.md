...
- [x] `/privacy` 页面上线，内容覆盖上述 6 项（占位结构）。
- [x] `/terms` 页面上线，内容覆盖上述 4 项（占位结构）。
- [x] 注册页有同意条款的明确步骤。
- [ ] 面向欧盟用户的部署版本有 Cookie 同意 banner（暂未实施，等真实欧盟用户出现再做）。
- [x] dev-log 记录上线时间，并附「下次法律审阅时间」提醒。

**完成时间**：2026-05-15（占位结构 + 注册流程）
