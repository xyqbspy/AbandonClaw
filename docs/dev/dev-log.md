...
### [2026-05-19] release-marginal-gaps M0 复核与 boot 自检落地

- 类型：Fast Track / 发布边际问题收口
- 状态：部分完成（代码侧完成，外部后台项待人工）

#### 背景

按 `docs/dev/release-marginal-gaps.md` 的 M0 优先级复核今天就该做的项。目标是不改业务主链路，只把仓库内能落地的防护收口到位，并把外部平台依赖如实标记为 blocked。

#### 本轮改动

- `instrumentation.ts` 新增启动期 `[boot]` 自检日志，Node runtime 启动时打印：
  - `sentry`
  - `upstash`
  - `resend`
  - `registrationMode`
  - `cspMode`
- 新增 `src/lib/server/boot-check.ts`，把 boot 自检快照抽成独立 helper，避免测试直接依赖 `instrumentation.ts` 顶层 Sentry 导出。
- 新增 `src/lib/server/boot-check.test.ts`，验证：
  - 只输出布尔 readiness 与模式值
  - env 缺失时默认回落到 `unset` / `report-only`

#### 验证结果

- `node --import tsx --test src/lib/server/boot-check.test.ts`：2/2 通过。
- `pnpm run text:check-mojibake`：通过。
- `pnpm run maintenance:check`：仍失败，但失败原因已收敛清楚：
  - `stabilize-auth-session-p0-smoke` 6.5 未完成
  - `harden-tts-warmup-p0p1` 6.5 未完成
- 当前工作区 `.env.local` 未加载：
...
- `盲点-2 boot 自检`：代码侧已完成，可在目标环境通过 `pm2 logs abandonclaw` 验证首行 `[boot]` 输出。
- `M0-1 Sentry DSN`：仍需用户在生产环境配置并做后台测试事件/告警验证。
- `M0-2 branch protection`：仍需用户在 GitHub Settings 后台启用并验证。
- `M0-3 去掉 maintenance:check continue-on-error`：当前不能做，因为 active changes 仍有 2 项未完成，贸然移除会把 CI 直接拉红。

### [2026-05-19] release-marginal-gaps 盲点-1：高成本用量快照执行入口

- 类型：Fast Track / 运维可观测性收口
- 状态：部分完成（仓库入口已落地，生产调度待接线）

#### 背景

`/api/admin/status` 已经能返回 `todayHighCostUsage.items`，但没有主动出口。为了不改 `/admin` UI、也不把 Sentry 日报配置硬编码进仓库，本轮先按 `release-marginal-gaps.md` 的路径 B 补一个独立脚本入口，供 PM2 cron 或手工执行。

#### 本轮改动

- 新增 `scripts/high-cost-usage-snapshot.ts`，加载 `.env.local` 后读取 `getTodayHighCostUsageSummary()`。
- 新增 `scripts/high-cost-usage-snapshot-lib.ts`，把 stdout 输出格式固定为：
  - 一行汇总 header
  - 每个 capability 一行明细
  - 一行 JSON 快照
- `package.json` 新增 `pnpm run usage:snapshot`。
- `docs/dev/release-marginal-gaps.md` 在盲点-1 下补充“仓库内已具备执行入口，现场调度待接线”。

#### 验证结果

- `node --import tsx --test scripts/high-cost-usage-snapshot-lib.test.ts`：待本轮验证。
- 实际 PM2 cron / 首日快照：待目标环境执行。

#### 结论

- 盲点-1 已从“只有文档建议”推进到“仓库内有稳定执行入口”。
- 是否每天自动产出快照，仍取决于目标环境是否接好 PM2 cron 或等价调度。

### [2026-05-19] release-marginal-gaps M0-3 解锁：归档 harden-tts-warmup-p0p1

- 类型：维护流程 / OpenSpec archive
- 状态：完成

#### 背景

`pnpm run maintenance:check` 之前因为 active changes 仍有 2 项未完成而失败：
- `stabilize-auth-session-p0-smoke` 6.5 是真人工冒烟登录（外部 blocked）。
- `harden-tts-warmup-p0p1` 6.5 是 "不提交，等待用户审核"，但代码已在 commit `0edf0e7` 合入 main、文档同步在 commit `c7d4b60`，task 状态在合入后早已无意义，纯流程残留。

#### 本轮改动

- `openspec/changes/harden-tts-warmup-p0p1/tasks.md` 6.5 改为已完成，摘要写明合入 commit 与文档同步 commit。
- `openspec/changes/harden-tts-warmup-p0p1/` 整体移动到 `openspec/changes/archive/2026-05-16-harden-tts-warmup-p0p1/`（归档目录按 archive 现有命名习惯 `YYYY-MM-DD-<change-id>`）。
- `release-marginal-gaps.md` M0-3 完成摘要更新：active changes 阻塞从 2 降到 1，并标注当用户完成 `stabilize-auth-session-p0-smoke` 6.5 人工冒烟并归档后，即可摘 `.github/workflows/ci.yml` 中 `maintenance:check` 的 `continue-on-error`。

#### 验证

- `pnpm run maintenance:check`：`openspec validate --all --strict` 已从 FAIL 变为 OK；OpenSpec active changes 阻塞从 2 降到 1（剩 `stabilize-auth-session-p0-smoke`）。

#### 结论

- M0-3 距离可摘标记又近一步，剩余动作完全外部依赖。
- 本次归档不触动主 specs（该 change 没有 spec delta）。

### [2026-05-19] release-marginal-gaps 盲点-2 深化：扩 boot-check 覆盖到关键 env
...
`getBootCheckSnapshot` 上一轮只查 5 项（sentry / upstash / resend / registrationMode / cspMode），但仍有几个会"静默降级 + 没人知道"的 env 漏在外面：
- `EMAIL_FROM` 缺失 → Resend 发件失败，注册验证码邮件发不出去。
- `EMAIL_VERIFICATION_CODE_SECRET` 缺失 → 退化到默认 secret，HMAC 强度归零。
- `APP_ORIGIN` / `NEXT_PUBLIC_APP_URL` / `NEXT_PUBLIC_SITE_URL` 三个候选别名全缺 → Origin 校验匹配失败概率大幅上升。
- `REGISTRATION_IP_LIMIT_MAX_ATTEMPTS/WINDOW_SECONDS` 缺失 → 用 3 次 / 600s 默认值，但生产是否真按默认运行没有可见性。
- `DAILY_QUOTA_*` 缺失 → 全部走服务端默认；启动时看不出来有几个被显式覆盖。

#### 本轮改动

- `src/lib/server/boot-check.ts` 新增 5 个字段：
  - `emailFrom` / `emailVerificationSecret` / `appOrigin`（布尔）
  - `registrationIpLimit`（`"custom"` / `"default"`）
  - `dailyQuotaOverrides`（被显式 set 的 quota env 数量，0..7）
- 抽 `DAILY_QUOTA_KEYS` 为模块常量并导出 `DAILY_QUOTA_KEY_COUNT`，避免测试与运行时口径漂移。
- `boot-check.test.ts` 补 4 个新测试：
  - 满配 / 全空两个基线测试断言 10 个字段全集。
  - `appOrigin` 三别名任一存在覆盖。
  - `registrationIpLimit` 任一限流 env 存在切 custom。
  - `dailyQuotaOverrides` 计数（含 whitespace 不计）。
  - 新增 secret 不泄露断言：把 sentry / upstash token / resend / verification secret 注入 env 后，snapshot 序列化不应出现原始 secret 子串。
- 所有字段都是 boolean / 枚举 / 计数，从设计上保证不打 secret 值。

#### 验证

- `node --import tsx --test src/lib/server/boot-check.test.ts`：6/6 通过。
- `instrumentation.ts` 启动入口未变，仍打 `[boot] {json}`，因此运维侧消费方式不变。

#### 结论

- 盲点-2 现在覆盖完整 env 漂移面：Sentry/Upstash/Resend/Email/Origin/IP 限流/Daily quota/Registration mode/CSP mode 全部一行 `[boot]` 日志可见。
- 下次 `pm2 reload` 后看一眼 `[boot]` 行即可判断生产 env 是否漂移。

### [2026-05-19] release-marginal-gaps M1-2 备料：Nginx 模板进仓库

- 类型：Fast Track / 部署可重复性
- 状态：完成

#### 背景

`release-marginal-gaps.md` M1-2 把 "Nginx 加 limit_req/limit_conn 基础规则" 放在 M1 档；`incident-response-runbook.md` §2.1 已经写了完整示例配置，但每次部署要从文档手工拼一份 .conf，容易：
- 漏拷 `limit_req_zone` 到 `http {}` 块（zone 必须放在 http，不是 server）。
- 漏带 `X-Forwarded-For` 等头，导致应用层 `getClientIp` 拿到 Nginx 的 127.0.0.1。
- 限流命中走默认 503，与应用层 429 行为不一致。

把配置写成可直接 cp 的模板，比每次抄文档更靠谱，也让首次部署能少踩坑。

#### 本轮改动

- `deploy/nginx.example.conf`：单文件 site 配置，包含 80→443 redirect + HTTPS server + `limit_conn` + `limit_req` + 高成本接口与 signup 单独限流 + 与应用层一致的 429 返回。文件头注释说明如何 cp 启用、为什么 zone 要放 http、为什么 high-cost 正则必须与 `src/lib/server/admin/high-cost.ts` 同步。
- `deploy/nginx.proxy_common.example.conf`：可复用 proxy 头模板，被主配置 include。明确标注 `X-Forwarded-For` 必须由 Nginx 注入。
- `deploy/README.md`：5 步启用清单（SSL → http zone → cp site → nginx -t → 冒烟），+ 限流阈值与应用层 env 的对应表，+ 失败时怎么办（指向 runbook §4 四类剧本）。
- `docs/dev/README.md`：在 release-marginal-gaps 之后追加 `deploy/` 入口指针。
- `release-marginal-gaps.md` M1-2 验收下追加 "仓库内已备料" 段落，标明可直接 cp 不用现场拼。

#### 验证

- `pnpm run text:check-mojibake`：待本段写完后跑。
- 模板未在生产 nginx 上启用（仓库内只是模板，启用需用户在 CVM 执行）。

#### 结论

- M1-2 从"建议要做"推进到"代码已备好，部署时一行 cp 就能套"，降低首次部署出错率。
- 实际启用 + 冒烟仍待用户在生产 CVM 执行；验收清单未勾。

### [2026-05-19] release-marginal-gaps M2-1 备料：CSP enforce 通过 env 控制
...
`release-marginal-gaps.md` M2-1 把 "CSP 切 enforce" 放在 M2 档，触发条件是 report-only 观察 1-2 周稳定。当时切 enforce 的唯一方式是手工改 `next.config.ts` 把 header key 从 `Content-Security-Policy-Report-Only` 改成 `Content-Security-Policy`，每次切都要走 PR + review + 部署，且有改错风险（比如顺手动了 directives）。

#### 本轮改动

- 新增 `src/lib/server/csp.ts`：抽出 `CSP_DIRECTIVES` 常量 + `isCspEnforce()` helper + `getCspHeader()` helper。`CSP_ENFORCE === "true"`（精确匹配，不接受 `1`/`TRUE`/带空格）时切到 `Content-Security-Policy`，否则保持 `Content-Security-Policy-Report-Only`。
- `next.config.ts` 改为 `import { getCspHeader } from "./src/lib/server/csp"`，把硬写的 header 替换为 helper 调用。directives 内容字节级不变。
- `src/lib/server/boot-check.ts` 的 `cspMode` 改为调 `isCspEnforce()`，确保启动日志与实际 header 口径一致。
- 新增 `src/lib/server/csp.test.ts`：6 个测试覆盖：
  - 默认 / `CSP_ENFORCE=true` 切 key。
  - 模糊匹配（`1` / `TRUE` / 带空格 / 空字符串）不应误切。
  - `isCspEnforce()` 与 `getCspHeader()` 决策一致。
  - `CSP_DIRECTIVES` 保留 supabase / upstash / sentry / open.bigmodel.cn 等必要 connect-src（防止后续误删）。
  - `report-uri /api/csp-report` 不能丢（否则违规收集失效）。

#### 验证

- `node --import tsx --test src/lib/server/csp.test.ts src/lib/server/boot-check.test.ts`：12/12 通过（CSP 6 + boot-check 6）。
- 手工跑 next.config.ts：默认输出 `Content-Security-Policy-Report-Only`；`CSP_ENFORCE=true` 输出 `Content-Security-Policy`；directives 始终带 `report-uri`。
- `pnpm run text:check-mojibake`：通过。

#### 结论

- M2-1 触发时只需在生产 env 加 `CSP_ENFORCE=true` 然后 `pm2 reload`，不需要改任何代码、不需要发 PR。降低事故概率。
- `cspMode` 字段在 `[boot]` 日志中已联动，运维侧切完之后看一眼 `[boot]` 行就知道是 enforce 还是 report-only。

### [2026-05-19] 本轮 release-marginal-gaps 推进小结

本轮共完成 4 项：
- M0-3 解锁（归档 `harden-tts-warmup-p0p1`，active changes 阻塞 2→1）。
- 盲点-2 深化（boot-check 覆盖从 5 项扩到 10 项，含 secret 不泄露断言）。
- M1-2 备料（`deploy/` 目录 + Nginx 模板 + proxy 头模板 + 启用文档）。
- M2-1 备料（CSP enforce 通过 `CSP_ENFORCE` env 切换 + 6 个测试）。

剩余 marginal-gaps 仍待外部执行：
- M0-1 / M0-2（Sentry DSN、GitHub branch protection）。
- M0-3 摘 `continue-on-error`（等 `stabilize-auth-session-p0-smoke` 6.5 人工冒烟）。
- M1-1（灾备演练）、M1-2 实际启用 nginx、盲点-1 接 PM2 cron、盲点-3 紧急开关演练。

仓库内的工程动作已经做满；下一阶段瓶颈全在生产 CVM / GitHub / Sentry 后台。

### [2026-05-19] docs/dev 文档收敛 audit 收尾
...
`docs/dev/` 内 release / public-registration 主题并列过多，策略、执行、baseline、边际项分散在多份文档里。当前主链路已稳定，继续并列维护只会增加 onboarding 成本和事实漂移风险。

#### 本轮改动

- 把 `docs/dev/release-readiness-assessment.md` 定为**唯一上线策略入口**：
  - 补入放行模式与推荐边界。
  - 吸收原 `release-marginal-gaps.md` 的 M0 / M1 / M2 / 盲点结论。
  - 去掉正文里对旧 release / public-registration 文档的现行依赖。
- 把 `docs/dev/backend-release-readiness-checklist.md` 定为**唯一上线执行清单**：
  - 直接补入真实 HTTP baseline 的 sample 复制、dry-run、全量执行、`passed / blocked / failed` 判读与留证口径。
  - 不再要求执行者先去读额外 runbook。
- 更新入口文档：
  - `docs/README.md`
  - `docs/dev/README.md`
  - `docs/dev/incident-response-runbook.md`
- 将以下旧文档收为**历史链接兼容占位页**，不再承载现行语义：
  - `docs/dev/release-marginal-gaps.md`
...
- `docs/dev/docs-audit-2026-05-15.md` 不重写：它是当时的 audit 快照，不追求改成当前现状说明。
- `docs/dev/dev-log.md` 旧条目不回写重命名：保留当时真实发生的文档名与上下文。
- 本轮不改业务代码、不改主链路、不改推荐 / 调度 / 学习行为语义。
...
- `docs/dev/` 的上线主题现已收敛为“两份主文档 + 四份兼容占位页”。
- 现行维护只应更新 `release-readiness-assessment.md` 与 `backend-release-readiness-checklist.md`；旧页只用于兼容历史链接。
