...
- 如果终端里看到 `CHANGELOG.md`、`docs/README.md` 或维护手册疑似乱码，先用 UTF-8 感知的读取方式或编辑器确认文件字节内容，再决定是否重写；不要把单个 shell 的显示异常直接当成文件损坏
- `scripts/check-mojibake.ts` 不得通过忽略自身来规避检查；如果需要维护高置信度乱码模式，优先使用编码安全表达并补最小验证
