...
匿名试用模块负责让未注册访客先体验公开场景学习的核心阅读层：场景列表、场景详情、句子气泡、翻译、相关短语详情、预生成 TTS 播放，以及固定练习 / 固定变体的本地体验。
...
- `/trial/scene/[slug]` 渲染匿名场景详情，视觉复用主 `/scene/[slug]` 的句子气泡、翻译、朗读、详情面板/弹层和当前下一步头部
- `/share/scene/[slug]` 继续作为分享灰度详情入口，使用同一匿名详情组件
- `练习` / `变体` 默认展示并视为全阶段解锁，点击进入固定本地 practice / variant fixture，不调用 AI 或写入接口
- 导入、生成、重新生成、提交、删除、表达地图、保存、加入复习、实时 AI 解释等入口只保留外观，点击显示注册阻断
...
- **允许**：公开场景列表读取、公开场景详情读取、公开 chunk/detail 展示、预生成 TTS 播放、固定 practice fixture 本地作答、固定 variant fixture 本地浏览
- **注册阻断**：导入场景、生成场景、重新生成练习、提交练习、完成/删除变体、生成表达地图、保存表达、加入复习、提交 review、写 progress、实时 AI 解释、实时 TTS 生成
...
- 匿名期间的浏览状态、详情打开状态、固定练习作答草稿、阻断弹窗状态只留在前端内存或 sessionStorage，不落用户态业务表
