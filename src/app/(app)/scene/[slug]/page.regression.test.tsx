...
function getNextStepActionButton() {
...
test("SceneDetailPage 当前任务区按标题卡片展示，问号入口点击后才显示训练浮层", async () => {
  const SceneDetailPage = getSceneDetailPage();
  render(<SceneDetailPage initialLesson={baseLesson} />);

  const nextStep = await screen.findByRole("region", { name: "当前下一步" });
  assert.ok(within(nextStep).getByRole("button", { name: "返回场景列表" }));
  assert.ok(within(nextStep).getByText("summary"));
  assert.ok(within(nextStep).getByRole("button", { name: "循环播放" }));
  assert.ok(within(nextStep).getByRole("button", { name: "执行当前下一步" }));
  assert.match(nextStep.textContent ?? "", /开始听整段/);
  assert.match(nextStep.textContent ?? "", /先把场景听熟一遍/);
  assert.match(nextStep.textContent ?? "", /下一步：\s*看 1 个重点表达/);
  assert.equal(within(nextStep).queryByRole("button", { name: /展开当前任务|折叠当前任务/ }), null);
  assert.doesNotMatch(nextStep.textContent ?? "", /完整进度和快捷入口保留/);
  const fab = await screen.findByTestId("scene-training-fab");
  assert.equal(within(nextStep).getByTestId("scene-training-fab"), fab);
  ...
  assert.ok(screen.getByRole("button", { name: "训练进度入口" }).querySelector('[data-progress-icon="info-circle"]'));
  assert.ok(screen.getByText(/1\. 听熟这段/));
...
test("SceneDetailPage 记录整段播放和打开表达后，会更新入口步骤文字", async () => {
  ...
  const nextStep = screen.getByRole("region", { name: "当前下一步" });
  fireEvent.click(within(nextStep).getByRole("button", { name: "循环播放" }));
  ...
  await waitFor(() => {
    assert.match(nextStep.textContent ?? "", /下一步：\s*解锁变体/);
  });
...
test("SceneDetailPage 到开始练习步骤但尚无题集时，只由主 CTA 负责生成", async () => {
  currentLearningState = buildLearningState({
    progress: {
      masteryStage: "scene_practice",
      masteryPercent: 80,
  ...
  await revealTrainingPanel();
  assert.equal(screen.queryByRole("button", { name: /生成并完成整段练习|继续整段练习|练习/ }), null);
  const nextStep = screen.getByRole("region", { name: "当前下一步" });
  fireEvent.click(within(nextStep).getByRole("button", { name: "执行当前下一步" }));
...
test("SceneDetailPage 已到练习阶段且已有题集时，会保留次级练习入口", async () => {
  currentGeneratedState = {
    latestPracticeSet: practiceSet,
    latestVariantSet: null,
    practiceStatus: "generated",
  ...
  await revealTrainingPanel();
  const nextStep = screen.getByRole("region", { name: "当前下一步" });
  fireEvent.click(within(nextStep).getByTestId("scene-stage-practice-entry"));
...
test("SceneDetailPage 已解锁且已有变体集时，会保留次级变体入口", async () => {
  currentGeneratedState = {
    latestPracticeSet: practiceSet,
  ...
  await revealTrainingPanel();
  const nextStep = screen.getByRole("region", { name: "当前下一步" });
  fireEvent.click(within(nextStep).getByTestId("scene-stage-variant-entry"));
...
test("SceneDetailPage 当前步骤已推进到开始练习时，浮层CTA会跟随到练习页而不是停留在已并入练习的旧步骤", async () => {
  ...
  fireEvent.click(getNextStepActionButton());
...
test("SceneDetailPage 从练习页返回时保留当前练习步骤，不回退到听熟这段", async () => {
  ...
  await revealTrainingPanel();
  assert.ok(getNextStepActionButton());
  assert.equal(screen.queryByRole("button", { name: /进入句子练习|继续整段练习/ }), null);
...
test("SceneDetailPage 生成场景练习时会锁定主 CTA，并保持加载文案稳定", async () => {
  ...
  await revealTrainingPanel();
  assert.equal(screen.queryByRole("button", { name: "生成并完成整段练习" }), null);
  const nextStep = screen.getByRole("region", { name: "当前下一步" });
  const nextStepButton = within(nextStep).getByRole("button", { name: "执行当前下一步" });
  fireEvent.click(nextStepButton);

  await waitFor(() => {
    assert.equal(nextStepButton.hasAttribute("disabled"), true);
    assert.match(nextStepButton.textContent ?? "", /练习准备中\.\.\./);
  });
  assert.equal((nextStepButton.textContent ?? "").includes("练习准备中...中..."), false);
...
test("SceneDetailPage 打开变体训练时会锁定主 CTA，并保持加载文案稳定", async () => {
  ...
  await revealTrainingPanel();
  assert.equal(screen.queryByRole("button", { name: "打开变体训练" }), null);
  const nextStep = screen.getByRole("region", { name: "当前下一步" });
  const nextStepButton = within(nextStep).getByRole("button", { name: "执行当前下一步" });
  fireEvent.click(nextStepButton);
  ...
  await waitFor(() => {
    assert.equal(nextStepButton.hasAttribute("disabled"), true);
    assert.match(nextStepButton.textContent ?? "", /变体准备中\.\.\./);
  });
  assert.equal((nextStepButton.textContent ?? "").includes("变体准备中...中..."), false);
...
test("SceneDetailPage 在已完成练习后，主CTA会推进到变体，但步骤列表仍保留复习入口", async () => {
  ...
  const nextStep = await screen.findByRole("region", { name: "当前下一步" });
  assert.ok(within(nextStep).getByText("打开变体训练"));
  await revealTrainingPanel();
  assert.equal(screen.queryByRole("button", { name: "打开变体训练" }), null);
...
test("SceneDetailPage 练习阶段不会回退显示英文模式名", async () => {
  ...
  await revealTrainingPanel();
  assert.match(screen.getByRole("region", { name: "当前下一步" }).textContent ?? "", /下一步：\s*解锁变体/);
