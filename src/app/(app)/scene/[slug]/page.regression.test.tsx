...
test("SceneDetailPage 主场景页默认只显示图标入口，展开后才显示训练浮层", async () => {
  const SceneDetailPage = getSceneDetailPage();
  ...
  assert.equal(screen.queryByText("本轮训练"), null);
...
test("SceneDetailPage 到开始练习步骤后，会在训练浮层里给出直接练习入口", async () => {
  ...
  await revealTrainingPanel();
  fireEvent.click(screen.getByRole("button", { name: /生成并开始练习|开始场景练习|练习/ }));
...
test("SceneDetailPage 当前步骤已推进到开始练习时，浮层CTA会跟随到练习页而不是停留在练核心句", async () => {
  currentLearningState = buildLearningState({
    progress: {
      masteryStage: "sentence_practice",
      masteryPercent: 60,
...
test("SceneDetailPage 到练核心句步骤后，会后台预热场景练习而不打断当前页面", async () => {
  currentLearningState = buildLearningState({
    progress: {
      masteryStage: "sentence_practice",
      masteryPercent: 60,
...
test("SceneDetailPage 生成场景练习时会锁定主 CTA，并保持加载文案稳定", async () => {
  ...
  await revealTrainingPanel();
  fireEvent.click(screen.getByRole("button", { name: "生成并开始练习" }));
...
test("SceneDetailPage 练习阶段提示会优先显示中文模式名", async () => {
  currentLearningState = buildLearningState({
    progress: {
      masteryStage: "scene_practice",
      masteryPercent: 80,
  ...
  await revealTrainingPanel();
  assert.ok(screen.getAllByText(hasTextContent("当前建议继续做半句复现，系统已记录 2 次作答。")).length >= 1);
