...
test("SceneDetailPage 当前任务区默认折叠，问号入口点击后才显示训练浮层", async () => {
  const SceneDetailPage = getSceneDetailPage();
  render(<SceneDetailPage initialLesson={baseLesson} />);
  ...
  assert.doesNotMatch(nextStep.textContent ?? "", /下一步：/);
  fireEvent.click(within(nextStep).getByRole("button", { name: "展开当前任务" }));
...
test("SceneDetailPage 本轮完成后才显示变体入口", async () => {
  ...
  const nextStep = screen.getByRole("region", { name: "当前下一步" });
  fireEvent.click(within(nextStep).getByRole("button", { name: "展开当前任务" }));
  assert.match(nextStep.textContent ?? "", /下一步：解锁变体/);
});
