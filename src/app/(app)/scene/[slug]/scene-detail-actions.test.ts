...
test("buildPracticeSet 会让 5 词句子进入 guided recall，并把半句模块上限放宽到 5 题", () => {
  const baseLesson = createLesson({ id: "base-1", title: "Base Scene" });
  const sourceLesson = createLesson({
    id: "variant-1",
    title: "Variant Scene",
    sourceType: "variant",
    sections: [
        ...
        blocks: [
          {
            ...
            sentences: Array.from({ length: 6 }, (_, index) => ({
              id: `sentence-${index + 1}`,
              text: `We should head home now ${index + 1}`,
              translation: `我们现在该回家了 ${index + 1}`,
              chunks: ["head home"],
            })),
          },
        ],
  ...
  const guidedRecallModule = practiceSet.modules?.find((module) => module.mode === "guided_recall");
  assert.equal(guidedRecallModule?.exercises.length, 5);
  assert.equal(guidedRecallModule?.exercises[0]?.cloze?.displayText?.includes("____"), true);
});
