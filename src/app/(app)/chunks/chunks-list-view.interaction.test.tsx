...
test("ChunksListView 会保留卡片展开、相似项展开和句子记录操作", () => {
  ...
  const openedSourceSceneSlugs: string[] = [];
  ...
  const view = render(
    <ChunksListView
      ...
      openExpressionDetail={(item) => openedDetailIds.push(item.userPhraseId)}
      startReviewFromCard={() => undefined}
      openExpressionMap={async () => undefined}
      openSourceScene={(slug) => openedSourceSceneSlugs.push(slug)}
  ...
  fireEvent.click(view.getAllByRole("button", { name: "来源场景" }).at(-1)!);

  assert.deepEqual(toggledCardIds, ["expr-1", "expr-1"]);
  assert.deepEqual(toggledSimilarIds, ["expr-1"]);
  assert.deepEqual(openedDetailIds, ["expr-1"]);
  assert.equal(openedComposerCount, 1);
  assert.deepEqual(openedSourceSceneSlugs, ["coffee-chat"]);
