...
test("useManualExpressionComposer 在有 assist 时会保存 base/similar/contrast 并批量 enrich", async () => {
    ...
    useManualExpressionComposer({
      ...
      deps: {
        ...
        savePhrasesBatchFromApi: async (payload) => {
          ...
          const startId = nextBatchUserPhraseId;
          nextBatchUserPhraseId += payload.items.length;
          return {
            items: payload.items.map((_: unknown, index: number) => ({
              created: true,
              phrase: {
                id: `phrase-${startId + index + 1}`,
                normalized_text: `item-${startId + index}`,
                display_text: `item-${startId + index}`,
              },
              userPhrase: { id: `batch-${startId + index}` },
              expressionClusterId: "cluster-1",
            })),
          };
        },
  ...
  assert.deepEqual(output, {
    reviewSessionExpressions: [
      ...
      { userPhraseId: "batch-2", text: "keep going" },
    ],
    usedAssist: true,
    mode: "save_and_review",
  });
