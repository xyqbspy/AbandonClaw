...
const createPhrase = (
  ...
  usageNote: overrides.usageNote ?? "多用于提醒别过度消耗",
...
test("buildFocusDetailViewModel 会合并已保存关系与簇成员，并去重和过滤对照派生项", () => {
  ...
  assert.deepEqual(
    viewModel.similarRows.map((row) => row.text),
    ["overwork yourself", "push yourself too hard"],
  );
  assert.deepEqual(
    viewModel.contrastRows.map((row) => row.text),
    ["pace yourself", "take it easy", "missing contrast"],
  );
...
test("buildFocusDetailViewModel 在 loading key 不匹配时不会误显示相关项加载中", () => {
  const focusRow = createPhrase({
    userPhraseId: "main-1",
    text: "call it a day",
    normalizedText: "call it a day",
  ...
  const viewModel = buildFocusDetailViewModel({
    ...
    clusterMembersByClusterId: new Map(),
    phraseByNormalized: new Map(),
    savedRelationLoadingKey: "another-row",
    isContrastDerivedExpression: () => false,
    getUsageHint: () => "今天先到这",
    getReviewActionHint: () => "复习一次",
    defaults,
...
test("buildFocusDetailViewModel 在无 savedItem 但有 assistItem 时会完整走 assist fallback", () => {
  const viewModel = buildFocusDetailViewModel({
    focusDetail: {
      text: "barely slept",
  ...
  assert.equal(viewModel.activeAssistItem?.text, "barely slept");
  assert.equal(viewModel.usageHint, "描述睡得很少");
  assert.equal(viewModel.typicalScenario, "熬夜后的自我描述");
  assert.equal(viewModel.semanticFocus, "睡眠不足");
  assert.equal(viewModel.reviewHint, defaults.reviewHintFallback);
  assert.equal(viewModel.isSavedRelatedLoading, false);
});

test("buildFocusDetailViewModel 会把新生成但未保存的同类候选也并进详情视图", () => {
  const focusRow = createPhrase({
    userPhraseId: "main-1",
  ...
  const savedSimilar = createPhrase({
    userPhraseId: "sim-1",
    text: "make it through the day",
    normalizedText: "make it through the day",
    expressionClusterId: "cluster-1",
  });
  ...
  const viewModel = buildFocusDetailViewModel({
    ...
    focusAssistData: createAssistResponse({
      similarExpressions: [
        { text: "make it through the day", differenceLabel: "更口语" },
        { text: "survive the day", differenceLabel: "更强调熬过去" },
      ],
    }),
    savedRelationCache: {},
    clusterMembersByClusterId: new Map([["cluster-1", [focusRow, savedSimilar]]]),
    phraseByNormalized: new Map([["make it through the day", savedSimilar]]),
    savedRelationLoadingKey: null,
    isContrastDerivedExpression: () => false,
    getUsageHint: (item) => `用法:${item.text}`,
    getReviewActionHint: (status) => `复习:${status}`,
  ...
  assert.deepEqual(
    viewModel.similarRows.map((row) => ({
      text: row.text,
      kind: row.kind,
      saved: Boolean(row.savedItem),
    })),
    [
      { text: "make it through the day", kind: "library-similar", saved: true },
      { text: "survive the day", kind: "suggested-similar", saved: false },
    ],
  );
});
