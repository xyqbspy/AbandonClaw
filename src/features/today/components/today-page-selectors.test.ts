...
test("today continue helper 会把已并入练习的旧步骤映射成开始练习", () => {
  ...
  assert.equal(getContinueLearningStepLabel(continueLearning), "开始练习");
  assert.match(getContinueLearningHelperText(continueLearning), /当前先开始练习/);
});
