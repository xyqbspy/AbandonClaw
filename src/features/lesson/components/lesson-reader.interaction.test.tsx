...
test("LessonReader 在移动端点击朗读按钮时不会误打开详情 sheet，并直接显示对话翻译", () => {
  mockMatchMedia(true);
  const lesson = createDialogueLesson();
  ...
  assert.ok(within(blockContainer).getByText("我正在努力跟上进度。你很快就会好的。"));
  assert.equal(within(blockContainer).queryByRole("button", { name: "翻译" }), null);
