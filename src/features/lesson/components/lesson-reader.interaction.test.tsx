...
test("LessonReader 在训练模式下切换当前句时，训练按钮文案会跟随当前句同步", async () => {
  ...
  await waitFor(() => {
    screen.getByRole("button", { name: "再练一次" });
    screen.getByText("You'll be fine soon.");
  });

  fireEvent.click(screen.getByText("I'm trying to catch up."));

  await waitFor(() => {
    screen.getByText("I'm trying to catch up.");
    screen.getByRole("button", { name: "练这句" });
  });
