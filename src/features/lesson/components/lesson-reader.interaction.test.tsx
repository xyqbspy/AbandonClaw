...
function getTrainingCard() {
  ...
  const card = label.closest("[data-current-training-sentence]");
...
test("LessonReader 在移动端训练模式下点击分组内具体句子时，当前训练句会精确跟随点击的那一句", async () => {
  ...
  await waitFor(() => {
    const sticky = screen.getByText("当前训练句").closest("[data-current-training-sentence]");
    assert.ok(sticky);
    assert.equal((sticky as HTMLElement).dataset.currentTrainingSentence, "This is the first sentence.");
  });
  ...
  await waitFor(() => {
    const sticky = screen.getByText("当前训练句").closest("[data-current-training-sentence]");
    assert.ok(sticky);
    assert.equal((sticky as HTMLElement).dataset.currentTrainingSentence, "This is the second sentence.");
  });
});
