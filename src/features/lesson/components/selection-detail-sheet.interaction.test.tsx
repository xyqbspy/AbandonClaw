...
test("SelectionDetailSheet 会处理句子区切换、朗读和相关短语交互", () => {
  const sentenceLoops: string[] = [];
  ...
  render(
    <SelectionDetailSheet
      ...
      onLoopSentence={(text) => sentenceLoops.push(text)}
  ...
  fireEvent.click(within(sentenceSection).getByRole("button", { name: "句子2" }));
  ...
  assert.deepEqual(sentenceLoops, ["I don't want to burn out."]);
...
test("SelectionDetailSheet 会处理短语详情 fallback、例句翻译和朗读交互", () => {
